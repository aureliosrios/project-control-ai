            ...
            <div>
              ...
              >
                ...
                {sincronicos.map(curso => (
                  <option key={curso.id} value={curso.registro} disabled={curso.cerrado} className="bg-[#0f172a] text-white">
                    {curso.nombre} ({curso.cerrado ? "Inscripciones cerradas" : `Inicio: ${curso.inicio}`})
                  </option>
                ))}
              </select>
            </div>
